...
<div id="mjointbox">
	...
	<td class="td3">
		<select name="limit">
		<?php for($i = 1; $i < 21; $i++):?>
		<option value="<?php echo $i?>"<?php if($wdgvar['limit']==$i || (!$wdgvar['limit']&&$i==8)):?> selected="selected"<?php endif?>><?php echo $i?>개</option>
		<?php endfor?>
		</select>
	</td>
	</tr>
	...
	<td class="td3">
		<select name="bnum">
		<?php for($i = 0; $i < 21; $i++):?>
		<option value="<?php echo $i?>"<?php if($wdgvar['bnum']==$i || ($wdgvar['bnum']==''&&$i==3)):?> selected="selected"<?php endif?>><?php echo $i?>개</option>
		<?php endfor?>
		</select>
	</td>
	</tr>
	<tr>
	<td class="td1">구분갯수</td>
	<td class="td2">:</td>
	<td class="td3">
		<select name="gnum">
		<?php for($i = 0; $i < 21; $i++):?>
		<option value="<?php echo $i?>"<?php if($wdgvar['gnum']==$i || ($wdgvar['gnum']==''&&$i==4)):?> selected="selected"<?php endif?>><?php echo $i?>개</option>
		<?php endfor?>
		</select>
	</td>
	</tr>
	...
	<td class="td3 shift">
		<select name="disp">
		<option value="0"<?php if($wdgvar['disp']=='0'):?> selected="selected"<?php endif?>>출력안함</option>
		<option value="1"<?php if($wdgvar['disp']=='1'):?> selected="selected"<?php endif?>>출력함</option>
		</select>
	</td>
	</tr>
	<tr>
...
function widgetCode()
{
	...
	if(f.gnum.value) widgetInfo+= "'gnum'=>'"+f.gnum.value+"',";
	if(f.disp.value) widgetInfo+= "'disp'=>'"+f.disp.value+"',";
...
function saveCheck(f)
{
	...
	<?php if(!$option):?>
	opener.createTile('440px','300px','0px','0px');
	<?php endif?>

	var bbsx = f.bbsid.value.split('^');

	opener.blocktitle[n] = (bbsx[1]?'최근게시물('+bbsx[1]+')':'최근게시물(전체)');
	opener.blockarray[n] = "<?php echo $swidget?>,bid^" + bbsx[0] + ",limit^"+f.limit.value+",sbjcut^"+f.sbjcut.value+",link^"+f.link.value+",title^"+f.title.value+",bnum^"+f.bnum.value+",gnum^"+f.gnum.value+",disp^"+f.disp.value;
