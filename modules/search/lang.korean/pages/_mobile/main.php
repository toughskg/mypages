...
$RCD = getDbArray($table['s_comment'],'site='.$s.' and hidden=0 and d_regis > '.$d['search']['date'].$_searchsql,'*','uid','asc',$recnum,$p);
$NUM = getDbRows($table['s_comment'],'site='.$s.' and hidden=0 and d_regis > '.$d['search']['date'].$_searchsql);
?>
<?php if($NUM):$result=true?>
<div id="s_comment">
...
<?php while($R=db_fetch_array($RCD)):?>
	...
	<div class="sbjbox">
		<a href="<?php echo getCyncUrl($R['cync'].',CMT:'.$R['uid'])?>#CMT" class="subject"><?php echo $R['subject']?></a>
		<?php if(strstr($R['content'],'.jpg')):?><img src="<?php echo $g['img_core']?>/_public/ico_pic.gif" class="imgpos" alt="사진" title="사진" /><?php endif?>
		<?php if($R['upload']):?><img src="<?php echo $g['img_core']?>/_public/ico_file.gif" class="imgpos" alt="첨부파일" title="첨부파일" /><?php endif?>
...
<div id="s_upload">
	...
	<?php while($R=db_fetch_array($RCD)):?>
	<tr>
	<td class="name"><a href="<?php echo $g['r']?>/?m=upload&amp;a=download&amp;uid=<?php echo $R['uid']?>" title="<?php echo $R['caption']?>"><?php echo $R['name']?></a></td>
