...
{
	getDbUpdate($table['s_comment'],$scoreset[$value].'='.$scoreset[$value].'+'.$score,'uid='.$R['uid']);
	if (!$UT['parent'])
	{
		getDbInsert($table['s_xtralog'],'module,parent,'.$scoreset[$value],"'".$m."','".$R['uid']."','".$scorelog."'");
	}
	else {
		getDbUpdate($table['s_xtralog'],$scoreset[$value]."='".$UT[$scoreset[$value]].$scorelog."'","module='".$m."' and parent='".$R['uid']."'");
	}
}
else {
	getLink('','','이미 반영된 글입니다.','');
